...
class Conv1dOp extends OpTrait {
  ...
  constructor({ stride, padding, padValue }: Conv1dOpsParams = {}) {
    super()
    this.padding = padding ?? 0

    assert(
      this.padding >= 0,
      `conv1d: padding must be greater or equal than 0, got ${this.padding}`,
    )

    this.stride = stride ?? 1
    assert(
      this.stride > 0,
      `conv1d: stride must be greater than 0, got ${this.stride}`,
    )
  ...
  async compute(input: MaybePromise<Tensor>, weight: MaybePromise<Tensor>) {
    ...
    //   transpose,
    // )
    this.savedIm2cols = im2col(inputMatrix, [ kernelSize! ], this.stride).T

    const W = pipe(
      /** [cout, k, cin] */
      await weightMatrix.permute([ 2, 0, 1 ]),
      /** [cout, k * cin] */
      v => v.reshape([ cout!, kernelSize! * cin! ]),
    )

    const res = (await W.matmul(this.savedIm2cols!)).T
  ...
  async gradient(
    ...
    const [ outGrad, input, weight ] = await Promise.all(
      [ grad, ...inputs ].map(d => Promise.resolve(d).then(d => d.raw)),
    )
    ...
    // update inputGrad
    /** [k * cin, cout] */
    const wT = weight.reshape([ kernelSize! * cin!, cout! ])
    /** [N, K, cin] */
    const colGrad = pipe((await wT.matmul(outGrad.T)).T, (v) => {
      /** v: [N, k * cin] */
      return v.reshape([ -1, kernelSize!, cin! ])
    })
    for (const [ idx, indices ] of this.mapping.entries()) {
      for (let index = 0; index < indices.length; ++index) {
        const i = indices[index]! - this.padding
        if (inRange(i, 0, input.shape[0]!)) {
          const grad = colGrad.slice(idx, index)
          inputGrad.set([ i ], await inputGrad.slice(i).add(grad))
        }
      }
    }
    ...
    // update weightGrad
    /** [k * cin, feat_out] */
    const weightGrad = (await this.savedIm2cols!.matmul(outGrad)).reshape([
      kernelSize!,
      cin!,
      cout!,
    ])

    return [ new Tensor(inputGrad), new Tensor(weightGrad) ]
  }
}

export async function conv1d(
  input: MaybePromise<Tensor>,
  weight: MaybePromise<Tensor>,
  opts?: Conv1dOpsParams,
) {
